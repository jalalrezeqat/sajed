...
@section('content')
    <div class="dir edit-user-profile">
        <a href="{{ url('admin/dashboard/profile/' . Auth::guard('admin')->user()->id) }}"><button
                class="btn btn-back-user btn-lg btn-success">رجوع</button></a>

        <div class="form-edit-user-profile">
            <form action="{{ route('admin.postChangePasswordadmin') }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
